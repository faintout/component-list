<template>
    <div class="hot-bar">
        <div class="time-gap-area">
            <div class="time-gap-item" v-for="(item, index) in hotTimeList" :key="item.time">
                <div class="time">{{ item.time }}</div>
                <div class="gap"></div>
            </div>
        </div>
        <div class="color-box">
            <div class="color-line" :style="{ backgroundImage: generateGradient() }" ref="colorLineRef"
                @mousedown="handleMousedown">
                <el-tooltip ref="toolTipRef" manual :visible="toolTipVisible" :stop-popper-mouse-event="false"
                    :content="hotTimeList[positionIndex]?.time">
                    <div @mouseenter="!mouseMove && displayTooltip()" @mouseleave="!mouseMove && hideTooltip()"
                        :style="{ 'left': positionIndex * step + '%' }" class="handle-bar-icon" src="">
                    </div>
                </el-tooltip>
            </div>
        </div>
...
// 设置移动的步数
const step = computed(() => 100 / (props.hotTimeList.length - 1))
// 鼠标是否按下
...
const generateGradient = () => {
    const maxValue = Math.max(...props.hotTimeList.map(hotTime => hotTime.value));
    const colors = props.hotTimeList.map((hotTime, i) => {
        // 计算当前的百分比
        const percentage = (hotTime.value / maxValue) * 100;
        // 根据百分比计算颜色
...
const setPosition = async (event) => {
    ...
    positionValue = Math.max(0, Math.min(positionValue, 100))
    //计算当前的步数索引
